...
%% the main repetition loop 
Num_run = 1;

for mc=1:Num_run
    P_k = StartingPosition;  % Current position
    P_k_store_entropy = [];
    P_k_store_entropy = [P_k_store_entropy; P_k];
    pos.x = P_k(1);
    pos.y = P_k(2);
    pos.z = P_k(3);
    ...
    thetaMean_store_entropy = [];
    Tracking_error_store_entropy = [];
    P_vari_store_entropy = [];
    conc_est_store_entropy = [];
    ...
    for i = 1: Numl
        ...
        %%
        % thetaPrev=theta;
        [theta, Wpnorm]=UpdatePF(D, theta, Wpnorm, pos, P_k_store_entropy,m,N,PF_Memory,domain);
        ...
        % hold on
        % plot3(P_k_store_entropy(:,1), P_k_store_entropy(:,2), P_k_store_entropy(:,3),'r--','LineWidth',2)
        ...
        for k = 1:5  %for all proposed locations
            ...
            for jj = 1:M
                for jjj = 1:MM
                    ...
                    entropy(k) = entropy(k) +(sum(zWpnorm.*log(zWpnorm+(zWpnorm==0))))/(M*MM);

                end
            end

         dualControlJ(k) = entropy(k);

        end
    %% 
    [val,ind] = min(dualControlJ);
    ...
    %% this makes sure the arm not moving backward
        for ii = 1: 5
            ...
            validNextIndex = true;
            for iii = 1:size(P_k_store_entropy,1)
                if  P_k_store_entropy(iii,1)==X_neigh && P_k_store_entropy(iii,2)==Y_neigh && P_k_store_entropy(iii,3)==Z_neigh
        ...
        %%
        thetaMean_store_entropy = [thetaMean_store_entropy; thetaMean];

        Tracking_error = norm(P_k - s_k);
        Tracking_error_store_entropy = [Tracking_error_store_entropy; Tracking_error];

        P_vari_store_entropy = [P_vari_store_entropy; P_vari];

        conc_est_store_entropy = [conc_est_store_entropy; conc_est];
    ...
    %%
    Tracking_error_mc_10_entropy = mean(Tracking_error_hist, 1);
    P_vari_mc_10_entropy = mean(P_vari_hist, 1);
...
save(fullfile(folder, 'P_vari_mc_10_entropy'), 'P_vari_mc_10_entropy');
save(fullfile(folder, 'Tracking_error_mc_10_entropy'), 'Tracking_error_mc_10_entropy'); 
