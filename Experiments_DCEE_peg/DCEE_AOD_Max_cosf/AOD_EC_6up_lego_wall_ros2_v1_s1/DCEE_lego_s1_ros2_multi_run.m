...
for mc=1:Num_run
    ...
    for i = 1: Numl
        ...
        %% Require next bext viewpoint, 1 goes on; 0 break 
        node_1_ReqNBV = ros2node('node_1_ReqNBV');
        node_2_ReqNBV = ros2node('node_2_ReqNBV');

        RNBVSub = ros2subscriber(node_2_ReqNBV, "/ReqNBV", "std_msgs/Float64", @ROS2RNBVCallback);
        global Req_NBV
        

        RNBVPub = ros2publisher(node_1_ReqNBV, "/ReqNBV", "std_msgs/Float64");
        RNBVPubmsg = ros2message(RNBVPub);
        RNBVPubmsg.data = 1;
        send(RNBVPub, RNBVPubmsg)

        % disp('Waiting for message...');
        % pause(1); % allow some time for messages to arrive
    ...
    %% this makes sure the arm not moving backward
        ...
        %% publish the next location of camera's pose in quaternion 
        posiPub = ros2publisher(node_1_Pk, "/Set_cam_pose", "geometry_msgs/Pose");
        posiPubmsg = ros2message(posiPub);

        posiPubmsg.orientation.w = quat(1);
        posiPubmsg.orientation.x = quat(2);
        posiPubmsg.orientation.y = quat(3);
        posiPubmsg.orientation.z = quat(4);


        send(posiPub, posiPubmsg)
